...
public final class NeoForgeLoaderAccess implements LoaderAccess {
    ...
    private static VersionChecker.CheckResult checkResult(@Nullable PublishedBuild update) {
        if (update == null) {
            return new VersionChecker.CheckResult(VersionChecker.Status.UP_TO_DATE, null, Map.of(), null);
        }

        VersionChecker.Status status = update.channel() == ReleaseChannel.RELEASE
                ? VersionChecker.Status.OUTDATED
                : VersionChecker.Status.BETA_OUTDATED;
        return new VersionChecker.CheckResult(status, new ComparableVersion(update.version().toDisplayString()),
                Map.of(), update.downloadUrl());
    }
    ...
    public void showUpdate(@Nullable PublishedBuild update) {
        try {
            ...
            ModList.get().getModContainerById(MarlowCrystal.MOD_ID)
                    .ifPresent(container -> results.put(container.getModInfo(), checkResult(update)));
        } catch (ReflectiveOperationException | RuntimeException e) {
            Log.warn("Could not show the update check in the mod list: " + e.getMessage());
        }
    }
}
*///?}
